...
export const awaitUsingRequestAnimation = (win, selector, timeout, selectFunc) => {
  return createPromise((resolve, reject) => {
    const execute = () => {
      const nodes = selectFunc(selector);
      if (isNonEmptyArray(nodes)) {
...
export default ((selector, selectFunc = selectNodes, timeout = MAX_POLLING_TIMEOUT, win = window, doc = document) => {
  const nodes = selectFunc(selector);
  if (isNonEmptyArray(nodes)) {
