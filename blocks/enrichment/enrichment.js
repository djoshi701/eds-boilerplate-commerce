...
export default async function decorate(block) {
  ...
  try {
    ...
    if (type === 'category') {
      const plpBlock = document.querySelector('.block.product-list-page') || document.querySelector('.block.product-list-page-dynamic');
      if (!plpBlock) {
        throw new Error('No product list page block found');
      }

      let categoryId = plpBlock.dataset?.category || getMetadata('category-id');
      if (!categoryId) {
