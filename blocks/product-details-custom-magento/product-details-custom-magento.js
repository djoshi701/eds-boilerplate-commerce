...
async function setJsonLdProduct(product) {
  ...
  // const brand = product?.find((attr) => attr.name === 'brand');
  const brand = {value:'test'}
...
class ProductDetailPage extends Component {
  ...
  async componentDidMount() {
    ...
    try {
      if (!window.getProductPromise) {
        window.getProductPromise = getProductFromMagento(this.props.sku);
      }
  ...
  onAddToCart = async () => {
    if (Object.keys(this.state.selection).length === (this.state.product.configurable_options?.length || 0)) {
    ...
    return html`
      <${Fragment} >
        ...
        <div class="product-detail-description">
          ...
          <div dangerouslySetInnerHTML=${{ __html: this.state.product?.description?.html }}></div>
        </div>
      <//>
    `;
  }
