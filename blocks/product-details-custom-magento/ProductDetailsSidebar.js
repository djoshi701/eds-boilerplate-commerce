...
function Options({ product, updateSelection, selection }) {
  ...
  const renderSwatches = (option) => {
    const { values } = option;
    const selectedValue = selection?.[option.attribute_code]?.id;

    return values.map((value) => {
      const attr = {};
      if (option?.attribute_code === 'fashion_color') {
        attr.style = {
      ...
      return html`<li key=${value.id} class="swatch">
          <button
            ...${attr}
            class="button"
            aria-selected=${value.uid === selectedValue}
            onClick=${() => updateSelection({ [option.attribute_code]: {id: value?.uid, label: value?.label} })}>${value.label}</button>
        </li>`;
    });
  };

  return options.map((option) => {
    const selectedTitle = selection?.[option.attribute_code]?.label;
    return html`<div class=${`sidebar-section ${option.attribute_code === 'fashion_color' ? 'color-selector' : 'sizes-selector'}`}>
      <h4>${selectedTitle ? `${option.label}: ${selectedTitle}` : option.label}</h4>
...
export default class ProductDetailsSidebar extends Component {
  ...
  canAddToCart() {
    return Object.keys(this.props.selection).length === (this.props.product.configurable_options?.length || 0);
  }
