...
function renderItem(unitId, product) {
  ...
  const item = document.createRange().createContextualFragment(`<div class="product-grid-item">
    <a href="/products/${urlKey}/${product.sku.toLowerCase()}">
      <picture>
        <source type="image/webp" srcset="${image}?width=600&format=webply&optimize=medium" />
        <img loading="lazy" alt="${product.name}" width="600" height="750" src="${image}?width=300&format=jpg&optimize=medium" />
      </picture>
      <h3>${product.name}</h3>
    </a>
  </div>`);
...
function renderItems(block, results) {
  ...
  if (!recommendation?.productsView?.length) {
    // Remove block content if no recommendations are available
    block.remove();
    return;
  }
